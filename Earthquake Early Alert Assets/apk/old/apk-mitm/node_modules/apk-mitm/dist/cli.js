...
const apktool_1 = require("./tools/apktool");
...
async function main() {
    ...
    const { taskFunction, skipDecode, isAppBundle, outputName } = await determineTask(inputPath);
    const outputPath = path.resolve(path.dirname(inputPath), outputName);
    // Initialize and validate certificate path
    let certificatePath;
    const mapsApiKey = args['maps-api-key'];
    if (args.certificate) {
    ...
    const apktool = new apktool_1.default({
        frameworkPath: path.join(tmpDir, 'framework'),
        customPath: args.apktool ? path.resolve(args.apktool) : undefined,
    });
    const uberApkSigner = new uber_apk_signer_1.default();
    showVersions({ apktool, uberApkSigner });
    if (skipDecode) {
        console.log(chalk.dim(`  Patching from decoded apktool directory:\n  ${inputPath}\n`));
    }
    else {
    ...
    taskFunction({
        ...
        if (!args['keep-tmp-dir']) {
            try {
                ...
                const ignoreError = process.platform === 'win32' && error.code === 'EBUSY';
                if (!ignoreError)
                    throw error;
            }
        }
 ...
 */
async function determineTask(inputPath) {
    ...
    if (fileStats.isDirectory()) {
        ...
        if (!(await fs.exists(apktoolYamlPath))) {
            throw new user_error_1.default('No "apktool.yml" file found inside the input directory!' +
...
function formatCommandError(error, { tmpDir }) {
    return (error
        ...
        .replace(/^.+brut\.common\.BrutException: could not exec.+$/gm, line => chalk `{dim ${line}}`));
}
function showHelp() {
    console.log(chalk `
 ...
 * has an unsupported extension. Exits with status 1 after showing the message.
 */
function showSupportedCertificateExtensions() {
    console.log(chalk `{yellow
...
function showVersions({ apktool, uberApkSigner, }) {
    console.log(chalk `
...
exports.showArmWarning = showArmWarning;
main();
